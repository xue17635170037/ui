...
public class ExpandableHorizontalLayout extends LinearLayout {
    private static final String TAG = ExpandableHorizontalLayout.class.getSimpleName();

    private ExpandSettings mExpandSettings;
    ...
    private ExpandScrolledParent mExpandScrolledParent;
    ...
    public ExpandableHorizontalLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public ExpandableHorizontalLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        ...
        mExpandSettings = new ExpandSettings();
        if(attrs!=null) {
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.ExpandableLayout);
            mExpandSettings.expandDuration = typedArray.getInt(R.styleable.ExpandableLayout_expDuration, ExpandSettings.EXPAND_DURATION);
            mExpandSettings.expandWithParentScroll = typedArray.getBoolean(R.styleable.ExpandableLayout_expWithParentScroll,false);
            mExpandSettings.expandScrollTogether = typedArray.getBoolean(R.styleable.ExpandableLayout_expExpandScrollTogether,true);
            typedArray.recycle();
        }
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if(mExpandSettings.expandWithParentScroll) {
            mExpandScrolledParent = ExpandUtils.getScrolledParent(this);
        }
    }
    ...
    private int getParentScrollDistance () {
        ...
        if(mExpandScrolledParent ==null) {
            return distance;
        }

        distance = (int) (getY()+getMeasuredHeight()+mExpandedViewHeight
                - mExpandScrolledParent.scrolledView.getMeasuredHeight());
        for(int index = 0; index< mExpandScrolledParent.childBetweenParentCount; index++) {
    ...
    private void verticalAnimate(final int startHeight, final int endHeight ) {
        ...
        mExpandAnimator.setDuration(mExpandSettings.expandDuration);
        if(mExpandState==ExpandState.EXPANDING&& mExpandSettings.expandWithParentScroll&&distance>0) {

            mParentAnimator = ExpandUtils.createParentAnimator(mExpandScrolledParent.scrolledView,distance, mExpandSettings.expandDuration);

            mExpandScrollAnimotorSet = new AnimatorSet();

            if(mExpandSettings.expandScrollTogether) {
    ...
    public void setExpandScrollTogether(boolean expandScrollTogether) {
        this.mExpandSettings.expandScrollTogether = expandScrollTogether;
    }

    public void setExpandWithParentScroll(boolean expandWithParentScroll) {
        this.mExpandSettings.expandWithParentScroll = expandWithParentScroll;
    }

    public void setExpandDuration(int expandDuration) {
        this.mExpandSettings.expandDuration = expandDuration;
    }
